...
import RoleSidebar from "../ui/RoleSidebar";
import RoleRightPanel from "../ui/RoleRightPanel";
...
export default function EmployerDashboard() {
  ...
  const PIE_COLORS = ['#10B981', '#3B82F6', '#F97316']; // green, blue, orange (matching ref)
  const BAR_COLOR = '#3B82F6';

  return (
    <div className="min-h-screen bg-gray-50">
      <TopHeader onToggleLeft={() => setLeftCollapsed(s => !s)} onToggleRight={() => setRightCollapsed(s => !s)} />

      <div className="max-w-[1450px] mx-auto flex">
        {/* Left */}
        {leftCollapsed ? <RoleSidebar role={role} collapsed /> : <RoleSidebar role={role} />}

        {/* Main */}
        <main className="flex-1 p-6">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-semibold">Employer Dashboard</h1>
            <div className="text-sm text-slate-500">Welcome back, <span className="font-semibold">{user?.FirstName || 'Recruiter'}</span></div>
          </div>
          ...
          <div className="bg-card-bg rounded-xl p-6 shadow-soft mt-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead><tr className="text-left text-muted-text border-b">
        ...
        {/* Right */}
        {rightCollapsed ? <RoleRightPanel role={role} collapsed /> : <RoleRightPanel role={role} />}
      </div>
    </div>
  );
}